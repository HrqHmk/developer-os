...
**Esta é uma escolha de trade-off, e não uma solução que elimina o problema.** A autoridade de publicação existe na Cloudflare em qualquer das formas avaliadas; nenhuma delas a dispensa. O que a forma escolhida oferece é um conjunto estreito e nomeável:

- verificação declarada em arquivo versionado, que passa por Pull Request e por leitura humana;
- verificações granulares e individualmente reexecutáveis, elegíveis a bloqueio por C2;
- publicação mantida no fornecedor de deploy, onde a autoridade já reside;
- **ausência de uma segunda credencial de publicação armazenada no GitHub**.

E o que ela aceita explicitamente: autoridade de conta no Workers Builds, raio de impacto contido pela conta dedicada, três estados de build distintos, parte da configuração fora do repositório e duas superfícies operacionais.

As duas alternativas puras perdem por motivos diferentes. Automação nativa isolada deixa o comando de build fora do controle de mudanças (concessão maior que a de C5 aqui) e colapsa typecheck, teste e falha de infraestrutura num único sinal (contra C2). Actions realizando o deploy **duplicaria** a autoridade de publicação num segundo fornecedor, sob token de longa duração, num cenário sem OIDC — sem reduzir o raio de impacto, que continua de conta.
...
- **A autoridade de publicação existe e é de conta.** A plataforma gera automaticamente um token com `Workers Scripts: edit` na conta, não restringível a um Worker, e o usa no trabalho que também executa código do repositório. **A única vantagem de segurança reivindicada pela forma escolhida é que essa autoridade não é duplicada num segundo fornecedor**, permanecendo revogável e substituível no lugar onde já reside. É uma vantagem pequena, e está declarada como pequena.

**Conta dedicada como pré-condição de C7.** A terceira exigência de C7 — a conta não abriga recurso cujo comprometimento seja pior que o do próprio site — é satisfeita, nesta implementação, por **uma conta Cloudflare dedicada exclusivamente ao Developer OS**. Não é recomendação operacional: é a forma pela qual C7 se cumpre aqui, e portanto pré-condição do primeiro deploy real. Antes dele, verificar:

1. a conta utilizada é dedicada ao Developer OS;
2. não existem nela recursos alheios ao projeto cujo comprometimento ampliaria o raio de impacto;
3. o token usado pelo Workers Builds tem apenas as permissões necessárias entre as disponíveis naquele momento;
4. permissões não necessárias — KV, R2, Routes enquanto não houver binding correspondente — são removidas quando tecnicamente possível;
5. qualquer expansão posterior do conteúdo da conta é confrontada com C7 (gatilho 11).

**Contas separadas para preview e produção não são requisito.** Recuperariam isolamento mais forte, mas ao custo de duas configurações, duas superfícies operacionais e maior complexidade de domínio e bindings — desproporcional a um projeto pessoal de mantenedor único ainda em fundação. Ficam registradas em Alternativas como caminho disponível se a conta dedicada deixar de bastar.

**Conta dedicada não substitui D9.** Ela contém o raio de impacto da automação (C7); não segrega segredos entre ambientes da aplicação. Dentro da conta dedicada continuará cabendo, quando o gatilho de D9 ocorrer, Worker de preview e Worker de produção, Wrangler Environments ou outro mecanismo — e nenhum deles endereça C7, pela razão registrada em C8.
...
- **A execução de build de código não revisado alcança autoridade de conta, e isso não é contornável nesta plataforma.** É o limite mais relevante do documento. O token gerado pela plataforma tem `Workers Scripts: edit` na conta e não é restringível a um Worker; o comando de deploy é uma string no dashboard, mas resolve por `node_modules`, que o repositório controla. Portanto **um build comprometido deve ser tratado como capaz de exercer a autoridade concedida ao mecanismo de Workers Builds, cujo raio de impacto inclui os Workers da conta — produção entre eles.** A formulação modela a ameaça pelo pior caso compatível com a autoridade disponível, e deliberadamente **não** afirma um detalhe de execução do fornecedor que não foi demonstrado. Nenhuma das formas avaliadas evita esse raio de impacto, porque a autoridade de publicação é de conta em todas elas. As contenções reais são três, e são de escopo e não de isolamento: só o mantenedor empurra branch, preview de fork está desabilitado por política (§5), e a conta é dedicada ao projeto (§3). **O vetor realista não é contribuidor hostil — é dependência comprometida executando no build.** Gatilhos 10 e 11.
...
- **Parte da configuração de build só existe no dashboard do fornecedor.** É a **principal concessão de C5** na implementação inicial — prevista pela própria redação da propriedade, que admite entrada registrada quando não for versionável, e não uma violação dela. Mitigação: manter os campos como indireção fina, de modo que a substância permaneça versionada, e registrá-los em `conventions.md`.
...
- **Não gera confiança falsa.** §5 e §6 declaram o estreitamento de D3, a concessão de C5, o raio de impacto real da automação e as incertezas não resolvidas, no mesmo espírito de T8 e de ADR-0006 §4.
...
- **A principal concessão de C5 tem custo real.** Parte das entradas de build vive fora do controle de mudanças, e alterá-las não passa por Pull Request nem por leitura humana — exatamente o que G2 e G3 existem para impedir. C5 admite a concessão em vez de fingir que ela não existe, mas admitir não é anular: é uma superfície de mudança que escapa ao fluxo do projeto.
...
8. **A configuração fora do controle de mudanças crescer além de indireção fina** — momento em que a concessão deixa de caber na redação de C5 e passa a violá-la, exigindo revisão da forma.
...
11. **A conta dedicada passar a hospedar recurso alheio ao Developer OS** — outro projeto, outro site, recurso independente, ou dado e infraestrutura cujo comprometimento seja pior que o do próprio site. C7 é satisfeita hoje precisamente porque **a conta inteira é o raio de impacto deliberadamente aceito do projeto**; deixar de ser verdade quebra a terceira exigência no ato. É a forma mais provável de a propriedade ser violada sem que ninguém perceba, porque é uma ação tomada fora do repositório e que nenhum diff registra.
...
- **Contas Cloudflare separadas para preview e produção** — a forma de recuperar isolamento real sob a granularidade atual da plataforma, já que a autoridade do build ficaria confinada à conta de preview. Descartada agora por **proporcionalidade**: duas configurações, duas superfícies operacionais e maior complexidade de domínio e bindings, para um projeto pessoal de mantenedor único ainda em sua fundação. A conta dedicada única entrega a contenção que C7 exige a um custo muito menor. Permanece disponível caso a conta dedicada deixe de bastar, o perfil de risco do projeto aumente, ou a granularidade da plataforma continue inadequada (gatilhos 10 e 11).
