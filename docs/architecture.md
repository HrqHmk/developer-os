...
Esta é a estrutura-alvo: diretórios nascem quando há código que os justifique (`architecture.md` §11), não antecipadamente. `src/routes/`, `src/content/` (ADR-0003, desde o Blog), `src/lib/`, `public/` e `src/components/` já existem. `public/` foi introduzido pelo favicon e pela imagem Open Graph da homepage (Launch Readiness v1, Issue #62): ativos globais que entram no formato final em que são servidos, copiados sem processamento para o diretório servido pelo Workers Static Assets (`conventions.md` §8.3). `src/components/` foi introduzido por `theme-control.tsx` (Dark Mode, Issue #44) e hoje também é usado por `search-panel.tsx` (Search v1, Issue #48), `newsletter-signup.tsx` (Newsletter v1, Issue #53) e `home-header.tsx` (Homepage Visual Refresh v1, Issue #57 — header exclusivo de `/`, não chrome global) — o critério é responsabilidade clara ou reutilização real (`conventions.md` §7), não antecipação. `src/integrations/` ainda não — aparece quando a necessidade correspondente (primeira integração externa) existir.
