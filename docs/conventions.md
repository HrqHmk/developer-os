...
- **Como a aplicação consome o pipeline sem alcançá-lo**: `vite.config.ts` chama a função de build de cada tipo (`buildArticles()`, `buildProjects()`, `buildChangelog()`) uma única vez cada, guarda os resultados em constantes próprias, e distribui cada snapshot para seu próprio plugin Vite (`virtualArticlesPlugin`/`virtualProjectsPlugin`/`virtualChangelogPlugin`, em `virtual-articles-plugin.ts`/`virtual-projects-plugin.ts`/`virtual-changelog-plugin.ts`), que apenas serializa o array recebido como o módulo virtual correspondente (`virtual:articles`/`virtual:projects`/`virtual:changelog`) — nenhum desses plugins importa os módulos do compilador (`build-*.ts`/`discovery.ts`/`frontmatter.ts`/`markdown.ts`) nem `node:fs`/`gray-matter`/`unified`. **Nem todo tipo contribui para `prerender.pages`** (§8.6): só tipos com rota dinâmica por entrada o fazem — hoje Article e Project. Changelog não tem `$slug`, então seu snapshot alimenta só o módulo virtual; sua única página é estática e alcançada pelo crawler do prerender, como `/uses`/`/architecture` já são. Rotas importam só o módulo virtual do seu próprio tipo. Essa fronteira é o que garante, estruturalmente, que o compilador de conteúdo nunca alcança o bundle do cliente nem o do servidor de requisições (P3) — verificado por inspeção do artefato de build, não apenas presumido. Os módulos virtuais permanecem **um por tipo de conteúdo**, deliberadamente não generalizados em um plugin genérico.
...
- Rotas dinâmicas de conteúdo (ex. `/blog/$slug`, `/projects/$slug`) não entram na descoberta automática de prerender (ADR-0001). `vite.config.ts` deriva `pages` concatenando os snapshots **dos tipos que têm rota dinâmica por entrada** — hoje `buildArticles()` e `buildProjects()` — nunca de uma listagem mantida à parte, e nunca só do crawler de links do prerender. Um tipo sem rota `$slug` (ex.: Changelog) não contribui para `pages`: sua única página é estática, e é descoberta pelo crawler do prerender a partir de um link alcançável — o mesmo mecanismo que já publica `/uses` e `/architecture`.
