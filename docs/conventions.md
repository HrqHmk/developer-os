...
- O pipeline vive em `src/content/pipeline/`, com **responsabilidades separadas por módulo**. `discovery.ts` (`discoverEntries(entriesDir)`, via `node:fs` — `readdirSync`/`statSync`, sem glob) e `frontmatter.ts` (`parseFrontmatter(raw, schema, context)`, extração via `gray-matter` + validação contra o `schema` recebido como parâmetro) são **compartilhados por todos os tipos de conteúdo** (Article, Project, Changelog) — nenhum dos dois módulos conhece qual tipo está servindo; cada `build-<tipo>.ts` fornece seu próprio diretório e seu próprio schema. `markdown.ts` (`toHtml(markdown)`, processamento via `unified`) é igualmente compartilhado e nem recebe schema — é agnóstico a tipo de conteúdo por definição.
- Cada tipo de conteúdo expõe **seu próprio ponto de entrada público**: cada `build-<tipo>.ts` (hoje `build-articles.ts`, `build-projects.ts`, `build-changelog.ts`) exporta sua própria função (`buildArticles()`, `buildProjects()`, `buildChangelog()`). Rotas e componentes **nunca importam arquivos `.md` diretamente**, nem `discovery.ts`/`frontmatter.ts`/`markdown.ts`, nem invocam o processador de Markdown por conta própria — nem sequer essas funções de build diretamente (ver abaixo).
...
- `buildChangelog(changelogDir?)` segue o mesmo contrato de determinismo e ausência de cache, e **ordena por `date` descendente** (mesmo comparador `localeCompare` do Article) — com uma diferença de leitura importante: `date` é uma data civil (`YYYY-MM-DD`), não um timestamp, então o comparador estabelece uma ordem determinística por **data, com slug como desempate quando as datas coincidem** — não uma ordem cronológica fina dentro do mesmo dia.
...
- **Como a aplicação consome o pipeline sem alcançá-lo**: `vite.config.ts` chama a função de build de cada tipo (`buildArticles()`, `buildProjects()`, `buildChangelog()`) uma única vez cada, guarda os resultados em constantes próprias, e distribui cada snapshot para seu próprio plugin Vite (`virtualArticlesPlugin`/`virtualProjectsPlugin`/`virtualChangelogPlugin`, em `virtual-articles-plugin.ts`/`virtual-projects-plugin.ts`/`virtual-changelog-plugin.ts`), que apenas serializa o array recebido como o módulo virtual correspondente (`virtual:articles`/`virtual:projects`/`virtual:changelog`) — nenhum desses plugins importa os módulos do compilador (`build-*.ts`/`discovery.ts`/`frontmatter.ts`/`markdown.ts`) nem `node:fs`/`gray-matter`/`unified`. **Nem todo tipo contribui para `prerender.pages`** (§8.6): só tipos com rota dinâmica por entrada o fazem — hoje Article e Project. Changelog não tem `$slug`, então seu snapshot alimenta só o módulo virtual; sua única página é estática e entra pela descoberta automática de prerender (§8.6), como `/uses`/`/architecture` já entram. Rotas importam só o módulo virtual do seu próprio tipo. Essa fronteira é o que garante, estruturalmente, que o compilador de conteúdo nunca alcança o bundle do cliente nem o do servidor de requisições (P3) — verificado por inspeção do artefato de build, não apenas presumido. Os módulos virtuais permanecem **um por tipo de conteúdo**, deliberadamente não generalizados em um plugin genérico.
...
- **Rotas dinâmicas** de conteúdo (ex. `/blog/$slug`, `/projects/$slug`) não entram na descoberta automática de prerender (ADR-0001) e por isso são enumeradas explicitamente: `vite.config.ts` deriva `pages` concatenando os snapshots **dos tipos que têm rota dinâmica por entrada** — hoje `buildArticles()` e `buildProjects()` — nunca de uma listagem mantida à parte.
- **Rotas estáticas não precisam de entrada em `pages`**: são incluídas pela **descoberta automática de prerender do TanStack Start**, a partir da própria route tree. Um tipo sem rota `$slug` (ex.: Changelog) não contribui para `pages` por esse motivo, assim como `/uses`, `/architecture` e `/search`.
- `buildArticles()`/`buildProjects()`/`buildChangelog()` rodam em contexto Node puro, fora do grafo de transformação do Vite — por isso a descoberta usa `node:fs`, não `import.meta.glob` (que não resolve dentro de `vite.config.ts`).

**Correção sobre o mecanismo das rotas estáticas.** Até a Issue #48 este documento afirmava que uma página estática era alcançada pelo **crawler de links** do prerender, a partir de um link em página já alcançável. Isso foi refutado por observação: na implementação de Search v1, `/search` foi prerenderizada (`dist/client/search/index.html`) **antes de existir qualquer link para ela** — a rota entrou pela descoberta automática, não pelo crawl. A consequência prática é sobre o que um link garante: um link em página alcançável continua necessário para **descoberta pelo visitante**, mas não é o mecanismo que produz o HTML prerenderizado, e sua ausência não deixa a página fora do build.
